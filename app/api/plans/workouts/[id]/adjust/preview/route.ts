...
import { previewSwap, previewMove, SwapType } from "@/lib/planAdjustment";
...
// Same body shape as the real /adjust endpoint — computes the impact without
// changing anything.
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.action === "swap") {
    ...
    const preview = await previewSwap(workoutId, body.workout_type as SwapType, {
  ...
  if (body.action === "move") {
    ...
    const preview = await previewMove(workoutId, body.new_date);
    return NextResponse.json(preview);
  }
